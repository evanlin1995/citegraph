...
app.config(['$routeProvider', '$locationProvider', function ($routeProvider, $locationProvider) {
  ...
  $routeProvider.when("/search", {
    ...
    controller: "SearchController" }).when("/graph/:query", {
...
var drawGraph = function drawGraph($scope) {
  ...
  for (var i = 0; i < paper.neighborsF.length; i++) {
    ...
    for (var j = 0; j < backNeighbors.length; j++) {
      if (backNeighbors[j] in $scope.theUI.nodes) {
        ...
        if (!(paper.neighborsF[i]._id in $scope.theUI.edges)) $scope.theUI['edges'][paper.neighborsF[i]._id] = {};
    ...
    for (var j = 0; j < frontNeighbors.length; j++) {
      if (frontNeighbors[j] in $scope.theUI.nodes) {
        ...
        if (!(frontNeighbors[j] in $scope.theUI.edges)) $scope.theUI['edges'][frontNeighbors[j]] = {};
  ...
  for (var i = 0; i < paper.neighborsB.length; i++) {
    ...
    for (var j = 0; j < backNeighbors.length; j++) {
      if (backNeighbors[j] in $scope.theUI.nodes) {
        ...
        if (!(paper.neighborsB[i]._id in $scope.theUI.edges)) $scope.theUI['edges'][paper.neighborsB[i]._id] = {};
