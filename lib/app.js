...
app.controller('GraphController', ['$scope', '$http', '$routeParams', function ($scope, $http, $routeParams) {
  ...
  $http.get('/paper/' + query).success(function (res) {
    ...
    $scope.getNodeName = function (id) {
      if (id < 0) return "";else if (id == res.paper.id) return res.paper.title;else if (!(id in res.neighbors)) return "";
    ...
    $scope.getYear = function (id) {
      ...
      var date = '';
      if (id == res.paper.id) date = res.paper.date;else if (id in res.neighbors) date = res.neighbors[id].d;else return "";
...
var drawGraph = function drawGraph($scope, paper, neighbors) {
  ...
  var maxFScore = -1,
      minFScore = Number.MAX_SAFE_INTEGER;
  paper.neighborsF.forEach(function (n) {
  ...
  var maxBScore = -1,
      minBScore = Number.MAX_SAFE_INTEGER;
  paper.neighborsB.forEach(function (n) {
    var score = getScore(curSketch, n.s);
  ...
  var maxNScore = -1,
      minNScore = Number.MAX_SAFE_INTEGER;
    ...
    if (backNeighbors) {
      ...
      for (var j = 0; j < size; j++) {
        if (backNeighbors[j] in theUI.nodes) {
          if (!(backNeighbors[j] in theUI.edges)) theUI['edges'][backNeighbors[j]] = {};
